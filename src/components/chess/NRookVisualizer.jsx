...
import { ArrowLeft, Play, Pause, RotateCcw, Castle } from 'lucide-react';

const NRookVisualizer = ({ onBack }) => {
  ...
  const startVisualization = async () => {
    ...
    try {
      const solutions = await solveNRooks();
      ...
      console.error('Error solving N-Rooks:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center mb-4">
            <Castle className="w-8 h-8 text-emerald-600 mr-3" />
            <h1 className="text-4xl font-bold text-gray-900">N-Rooks Visualizer</h1>
          </div>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Place N rooks on an N×N chessboard so that no two rooks attack each other
          </p>
        </div>
          ...
          {totalSolutions > 0 && (
            <div className="text-center mb-6">
              <p className="text-lg font-medium text-gray-700">
                For a {n}×{n} board, there are <span className="font-bold text-emerald-600">{totalSolutions}</span> possible solutions
              </p>
              ...
              {boards.slice(0, Math.min(8, boards.length)).map((boardData, boardIndex) => (
                <div
                  ...
                  className={`bg-white rounded-xl shadow-lg p-4 transition-all duration-300 ${
                    boardIndex === currentSolution ? 'ring-4 ring-emerald-400 scale-105' : ''
                  }`}
                >
...
export default NRookVisualizer;